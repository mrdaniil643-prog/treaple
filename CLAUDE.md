...
ADMIN_TOKEN=... TARGET=http://localhost:3000 python3 scripts/security-check.py   # 68 атак OWASP против dev-копии (создаёт тестовые данные)
...
**Доступ.** Заказом управляют по `secret` (передаётся во фрагменте URL `/tickets#order=…`, не в query) или по номеру заказа + телефону. Публичная ссылка на билет (`getTicket`) намеренно не отдаёт номер заказа, контакты и чужие билеты. Контролёр получает урезанный `gateView`, админ — полный `ticketView`.
